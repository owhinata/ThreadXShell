...
8d. **grove-vision-ai-v2: Himax SDK は read-only、ThreadX は Secure 単一モード、
   ...
   Zbit ZB25LQ128C**（#89。刻印と JEDEC `5e 50 18` で一致）。耐久・消去単位は未確認。自動ループ焼き不可。復旧 =
   ...
   **NPU 側へ戻すと `nn close` が IRQ 133 を disable する**（#86 の欠陥そのもの）。
   ...
   `0x200000..0xB70000` は **blob 予約**（9,895,936 B）＋ `0xD50000..0xFF0000` が
   **blob-tail**（2,752,512 B）で、書き手はまだ無い。**2 分割なのは間にモデルがあるからで、
   ...
   **最終ブロック `0xFF0000..0x1000000` は `slot-header` 予約で絶対に書かない** —
