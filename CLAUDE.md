...
- [!] **plugin container（#108 = 3a / #110 = 3b / #116 = 3c）**: `nn model load --slot` は
  ...
  **結果リース**（`port/plugin/plugin_lease.c`）で囲う: 順序は**常にリース → フレームロック**、
  ...
  飛ばし、それを数える）。**plugin に入る経路は全部取る** — param（`nn thresh`）・
  admission・load/unload も含む（セッションは worker しか排除しない）。
  **リース保持は「描いてよい」ではない**: overlay スイッチと記録の valid / kind も見る。**report は snapshot と同じ保護区間で採取し、バッファは
  ...
  **[!] スタックは 2 つの量**（呼び出し地点の空き / veneer の下でファームが使う量）。
  後者は **#110 で 512 B と実測し 640 を宣言**（過大が安全側）— **ベースに callback が
  増えたら導出し直す**。**コールバック自身のフレームだけでは「crossing の下」にならない**
  （初版は除算ヘルパ 2 本を落として 112 B 不足）。**ファームは古い宣言を見分けられない**
  ので、コストを変えたら既存 container は送り直す（恒久対処は ABI 変更で別 Issue）。**どのスレッドで呼ばれるかを間違えない**: entry / shapes_ok /
...
- **[!] plugin container（#101 = Step 1a / #103 = Step 1b）**: モデルと、その出力を
  ...
  - **[!] painter の予算はガード保持時間に比例する仕事の上界**であって `draw()` 内の
    ...
    **[!] 輪郭は「書く画素数」で課金する**（#105。外接面積だと近距離の顔 1 つ
    ...
    同じ列を 2 度書く。**共有するのは幾何規則（`svc/rect_geom.c` の `rect_geom_norm`）
    だけで期待値は共有しない**: テストは**実ループが発行した store 数**を数え、golden 値も
    別に置く（`lcd_rect.c` はそのためにホストへリンクできる TU に分離し、#110 で幾何だけ
    さらに `svc/` へ出した。**`-O3` は動かすたびに一緒に運ぶ**）。
