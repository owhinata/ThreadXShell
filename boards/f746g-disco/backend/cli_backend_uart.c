...
/*
 * printf from a thread whose console is NOT this backend: the telnet instance
 * (port/netxduo/nx_shell.c), or a background job launched from it -- cli_job.c
 * aliases a worker's tr to the foreground's.  Its bytes must not land on the VCP
 * ring.  Send them through the core's staged output path on that instance's own
 * transport, so printf follows the terminal that ran the command exactly like
 * cli_print does.
 *
 * The shared core states the rule this implements as a contract ("printf must
 * never silently misroute", shell/core/cli_core.c), and it is not cosmetic: the
 * multi-kilobyte CoreMark report is printf output (ee_printf == printf), so
 * without this a `coremark` typed at the telnet console produces a telnet session
 * that appears to hang while the whole report scrolls past on the VCP.
 *
 * That path is flow-controlled (cli_tx_send_blocking waits for TX space), unlike
 * the ring fast path below which is deliberately best-effort -- which is what a
 * report of that size needs in order to arrive intact.  Same LF -> CRLF
 * translation, and the same "return len" contract: output is never re-sent.
 * Adapted from boards/wio-lite-ai/backend/cli_backend_usbcdc.c, where the same
 * handoff has been in service since that board grew its second console.
 */
static int uart_write_other(struct cli_instance *sh, const char *ptr, int len)
{
	uint8_t prev = 0;
	int i;

	if (cli_out_begin(sh) != 0)
		return len;             /* lock unavailable: drop, as the ring path does */
	for (i = 0; i < len; i++) {
 ...
 * editor and inherits the bg line-break.  A thread that owns an instance on a
 * DIFFERENT backend goes to uart_write_other() above instead.  From an ISR,
 * before the scheduler, or from a non-shell thread there is no instance at all,
 * no lock is taken and we fall back to g_uart_console (byte-for-byte as before).
...
int _write(int file, char *ptr, int len)
{
	...
	/* Owned by another backend's console (the telnet instance): hand it over
	 * rather than writing someone else's output to the VCP.  Checked BEFORE the
	 * g_uart_console fallback, which is now reached only when there is no owning
	 * instance at all -- ISR, pre-kernel, or a non-shell thread. */
	if (sh != NULL && sh->tr != NULL && sh->tr->api != &cli_uart_api)
		return uart_write_other(sh, ptr, len);
