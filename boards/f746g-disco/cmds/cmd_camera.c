...
#include "fs_devices.h"       /* fs_sd_device() / fs_qspi_device() (camera save) */
#include "cmd_xfer.h"
...
CLI_SUBCMD_SET_CREATE(camera_set_subcmds,
	CLI_CMD_ARG_USAGE(brightness, NULL, "<-4..4>", "<-4..4>",
	                  cmd_set_brightness, 2, 0),
	CLI_CMD_ARG_USAGE(contrast,   NULL, "<-4..4>", "<-4..4>",
	                  cmd_set_contrast, 2, 0),
	CLI_CMD_ARG_USAGE(saturation, NULL, "<-4..4>", "<-4..4>",
	                  cmd_set_saturation, 2, 0),
	CLI_CMD_ARG_USAGE(hue,    NULL, "<-180..150> in 30 deg steps", "<-180..150>",
	                  cmd_set_hue, 2, 0),
	CLI_CMD_ARG_USAGE(awb,    NULL, "<auto|sunny|office|home|cloudy>",
	                  "auto|sunny|office|home|cloudy", cmd_set_awb, 2, 0),
	CLI_CMD_ARG_USAGE(effect, NULL, "<none|bw|sepia|negative|blue|red|green>",
	                  "none|bw|sepia|negative|blue|red|green",
	                  cmd_set_effect, 2, 0),
	CLI_CMD_ARG_USAGE(flip,   NULL, "<none|mirror|flip|both>",
...
CLI_SUBCMD_SET_CREATE(camera_subcmds,
	...
	CLI_CMD_ARG_USAGE(res, NULL, "set resolution <qqvga|qvga|vga|wvga>",
	                  "qqvga|qvga|vga|wvga", cmd_camera_res, 2, 0),
	CLI_CMD_ARG_USAGE(format, NULL,
	                  "set pixel format <rgb565|yuv422|y8|jpeg> (jpeg: snapshot <=VGA)",
	                  "rgb565|yuv422|y8|jpeg", cmd_camera_format, 2, 0),
	CLI_CMD_ARG_USAGE(fps, NULL, "set frame rate <15|30> (30 needs `lcd off`)",
	                  "15|30", cmd_camera_fps, 2, 0),
	CLI_CMD_ARG_USAGE(capture, NULL,
	                  "snapshot the current mode + stats ('test' = colorbar)",
	                  "[test]", cmd_camera_capture, 1, 1),
	CLI_CMD_ARG_USAGE(save, NULL, "write frame raw <sd|fs> <path> (format per mode)",
	                  "sd|fs <path>", cmd_camera_save, 3, 0),
	CLI_CMD_ARG_USAGE(send, NULL, "stream the frame to the PC over YMODEM [name]",
	                  "[name]", cmd_camera_send, 1, 1),
	CLI_CMD_ARG_USAGE(set,  camera_set_subcmds,
	                  "OV5640 image quality (no arg = show current)",
	                  "[<setting> <value>]", cmd_camera_set, 1, 1),
