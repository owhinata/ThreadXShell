...
/**
 * @file    guix_display.h
 * @brief   GUIX 565rgb display driver bound to the LTDC double buffer
...
/* Live-preview geometry (owhinata/stm32f746g-disco#56): QVGA RGB565 drawn native 1:1,
   centred on
   ...
   port preserves the port <- ui dependency direction
   (owhinata/stm32f746g-disco#43/#61). */
...
/* Largest preview the UI can select (owhinata/stm32f746g-disco#69/#84): QVGA (320x240)
   -- 480x272 was
   removed (horizontal stretch from the sensor's 4:3 FOV + SDRAM overrun,
   owhinata/stm32f746g-disco#84).  The view buffer is sized to this so a resolution
   switch never reallocates; the live geometry is passed to
   guix_display_cam_preview_begin() per session. */
 ...
 *  Used by the camera live-preview sink (owhinata/stm32f746g-disco#56) to copy a ring
 * slot into its private view buffer; both must be DMA2D-coherent (non-cacheable
 * SDRAM). */
int guix_display_copy_rgb565(uint16_t *dst, const uint16_t *src, uint32_t w,
                             uint32_t h, uint32_t dst_off, uint32_t src_off);

/* ---- camera live-preview copy-forward elimination (owhinata/stm32f746g-disco#59 Lever B2) --- */
 ...
 *  preview resolution (owhinata/stm32f746g-disco#69).  Both LTDC buffers start "stale"
 * so the first frames are corrected before present.  Returns 0, or -1 if the geometry
 * is invalid (NULL buffer / zero size / outside the 480x272 panel), leaving B2
 * disarmed.  Call from the camera UI (ui/) before the stream starts; runs under
 * ltdc_lock. */
 ...
 *  with true when the preview starts and false when it stops
 * (owhinata/stm32f746g-disco#61: the camera is the sole screen, so this tracks the
 * preview lifecycle; pre-owhinata/stm32f746g-disco#61 it tracked the camera screen
 * SHOW/HIDE). Runs under ltdc_lock. */
void guix_display_cam_set_visible(bool on);

/** Store a freshly captured frame (@p src, the current preview resolution RGB565,
 *  owhinata/stm32f746g-disco#69) into the view buffer via DMA2D and mark BOTH LTDC
 * buffers stale, atomically under ltdc_lock.  Returns 0 on a completed copy, -1
 * otherwise. */
 ...
 *  normalized [0,1] box @p nx,@p ny,@p nw,@p nh (owhinata/stm32f746g-disco#83
 * face-detect overlay). The box rides the view buffer, so it is composited into the
 * live image the same way as the frame itself (owhinata/stm32f746g-disco#59 B2) and is
 * overwritten by the next store. No-op if no preview session is armed (cam_view_data
 * NULL). Runs under ltdc_lock and marks both LTDC buffers stale.  Call after
 * guix_display_cam_view_store(), on the producer. */
