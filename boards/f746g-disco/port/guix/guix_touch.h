...
/**
 * @file    guix_touch.h
 * @brief   FT5336 -> GUIX pen-event input driver (owhinata/stm32f746g-disco#55,
 ...
 * It is EXTI-wake driven (owhinata/stm32f746g-disco#62): with no finger down it
 * blocks on the FT5336 INT wake (touch_wait_event) at ~0 % CPU, and only while a
 * finger stays down does it poll ~60 Hz to follow a drag (the controller emits no
 * edges during sustained contact).  Coordinates are panel pixels straight from
 * touch_read() (no scaling -- see touch.h).  The thread is created once and then
