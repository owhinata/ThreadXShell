...
/*
 * The candidate scratch, in the SDRAM AI carve-out it has always been in
 * (bank 3, MPU-cacheable, CPU-only).
 *
 * [!] ONLY THE SCRATCH GOES HERE.  `.sdram` is NOLOAD, so an object with
 ...
 * state therefore stays in ordinary internal RAM below.
 *
 ...
 *
 * [!] AND THIS BOARD HAD NO GATE WATCHING IT.  check_f746_layout.py's required
 * list never mentioned the decoder's buffers, `.sdram.ai` uses KEEP so
 * --gc-sections cannot even hint, and the linker ASSERTs bound only the section's
 * START -- an empty `.sdram.ai` passes them all.  Losing the attribute here would
 * have moved 1.5 KB into internal SRAM with every check still green.  board.cmake
 * now names this symbol; see the note there.
 */
