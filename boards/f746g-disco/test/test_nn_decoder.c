 ...
 *
 * [!] THE PLACEMENT IS CHECKED AT THE OTHER END.  The candidate scratch carries
 * section(".sdram.ai"), and check_f746_layout.py now names `nn_dec_scratch` on
 * the linked image -- a require this board did NOT have before issue #97, on a
 * board where losing the attribute would have moved the buffer into internal SRAM
 * with every existing check still green.  This file compiles the adapter; that
 * gate places it.
 *
 ...
 *     multiply every value by zero, and its graphs are float32.
