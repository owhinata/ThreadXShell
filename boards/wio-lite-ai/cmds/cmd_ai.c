...
CLI_SUBCMD_SET_CREATE(ai_stream_subcmds,
	CLI_CMD_ARG_USAGE(start, NULL, "claim the band stream and infer continuously [test]",
	                  "[test]", cmd_ai_stream_start, 1, 1),
...
CLI_SUBCMD_SET_CREATE(ai_model_subcmds,
	CLI_CMD_ARG_USAGE(load,   NULL, "read a .tflite from NOR blob <slot> and run it",
	                  "<slot>", cmd_ai_model_load,   2, 0),
...
CLI_SUBCMD_SET_CREATE(ai_subcmds,
	CLI_CMD_ARG(info,  NULL, "backend / model / tensor shapes / arena",
	            cmd_ai_info,  1, 0),
	CLI_CMD_ARG_USAGE(model, ai_model_subcmds,
	                  "runtime model swap (load <slot> / unload)",
	                  "[load <slot> | unload]", cmd_ai_model, 1, 2),
	CLI_CMD_ARG_USAGE(bench, NULL, "run inference [n] times, report latency",
	                  "[count]", cmd_ai_bench, 1, 1),
	CLI_CMD_ARG_USAGE(out,   NULL, "dequantized values of output tensor [i] [n]",
	                  "[tensor] [count]", cmd_ai_out,   1, 2),
	CLI_CMD_ARG(dets,   NULL, "decode the current outputs into face boxes",
	            cmd_ai_dets,   1, 0),
	CLI_CMD_ARG_USAGE(thresh, NULL, "score threshold [milli-probability 1..999]",
	                  "[1..999]", cmd_ai_thresh, 1, 1),
#if BSP_ENABLE_CAMERA
	CLI_CMD(run, NULL, "one-shot: grab a camera frame, infer, print the boxes",
	        cmd_ai_run),
	CLI_CMD_ARG_USAGE(stream, ai_stream_subcmds,
	                  "continuous camera inference (start [test] / stop / stats)",
	                  "[start [test] | stop | stats]", cmd_ai_stream, 1, 2),
	CLI_CMD_ARG_USAGE(norm, NULL, "input normalization [0=[0,1] | 1=[-1,1]]",
	                  "[0|1]", cmd_ai_norm, 1, 1),
#if BSP_ENABLE_LCD
	CLI_CMD_ARG_USAGE(overlay, NULL, "draw the boxes on the LCD preview [on|off]",
	                  "[on|off]", cmd_ai_overlay, 1, 1),
#endif
#endif
	CLI_SUBCMD_SET_END);
