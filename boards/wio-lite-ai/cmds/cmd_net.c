...
static void net_print_nx_ip(struct cli_instance *sh, const struct nx_net_info *ni)
{
	 ...
	 * automatically (issue #30 B2a) -- say so where the address is shown, because
	 * everything above it still LOOKS configured. */
	const char *stale = ni->lease_stale
	                  ? "  <-- STALE (link went down; run `net dhcp`)" : "";

	/* gw 0 is "no gateway", not an address (issue #19): printing 0.0.0.0 reads as
	 * a configured one. */
	if (ni->gw == 0u)
		cli_print(sh, "gw:    none (%s)%s\r\n",
		          ni->dhcp_mode ? "dhcp" : "static", stale);
	else
		cli_print(sh, "gw:    %u.%u.%u.%u (%s)%s\r\n",
		          (unsigned)((ni->gw >> 24) & 0xFFu),
		          (unsigned)((ni->gw >> 16) & 0xFFu),
		          (unsigned)((ni->gw >> 8) & 0xFFu),
		          (unsigned)(ni->gw & 0xFFu),
		          ni->dhcp_mode ? "dhcp" : "static", stale);
}
...
CLI_SUBCMD_SET_CREATE(net_shell_subcmds,
	CLI_CMD_ARG_USAGE(start,  NULL, "arm the telnet console [port] (default 23)",
	                  "[port]", cmd_net_shell_start, 1, 1),
...
CLI_SUBCMD_SET_CREATE(net_subcmds,
	...
	CLI_CMD_ARG_USAGE(ip, NULL, "set static <a.b.c.d/mask> [gw]",
	                  "<a.b.c.d/mask> [gw]", cmd_net_ip, 2, 1),
	CLI_CMD_ARG(dhcp, NULL, "(re)acquire an address via DHCP",    cmd_net_dhcp, 1, 0),
	CLI_CMD_ARG_USAGE(ping, NULL, "ICMP echo <a.b.c.d> [count]",
	                  "<a.b.c.d> [count]", cmd_net_ping, 2, 1),
	CLI_CMD_ARG_USAGE(echo, NULL,
	                  "TCP echo server [port] on the host stack (Ctrl+C stops)",
	                  "[port]", cmd_net_echo, 1, 1),
