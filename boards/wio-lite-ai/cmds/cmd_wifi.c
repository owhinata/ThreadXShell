...
CLI_SUBCMD_SET_CREATE(wifi_subcmds,
	...
	CLI_CMD_ARG_USAGE(log, NULL,
	                  "bridge LOG UART (UART9 @115200); `log reset` captures boot from t=0",
	                  "[reset]", cmd_wifi_log, 1, 1),
	CLI_CMD_ARG(ver,   NULL, "prove the eRPC link + read the FW build id (required before `wifi connect`)",
	            cmd_wifi_ver, 1, 0),
	CLI_CMD_ARG_USAGE(connect, NULL, "associate with an AP: connect <ssid> [pw] [sec_hex]",
	                  "<ssid> [pw] [sec_hex]", cmd_wifi_connect, 2, 2),
	CLI_CMD_ARG(disconnect, NULL, "drop the current WiFi association",     cmd_wifi_disconnect, 1, 0),
	CLI_CMD_ARG_USAGE(autoreconnect, NULL,
	                  "re-associate by ourselves when the AP goes away: autoreconnect [on|off]",
	                  "[on|off]", cmd_wifi_autoreconnect, 1, 1),
	...
	CLI_CMD_ARG_USAGE(chplan, NULL,
	                  "regulatory domain: chplan [set <hex> confirm] (writes PERSIST)",
	                  "[set <hex> confirm]", cmd_wifi_chplan, 1, 3),
