 ...
 * the linker.  DTCM is on the Cortex-M7 TCM interface (uncached; MPU/cache
 * maintenance N/A) and its bytes carry across every *system* reset (reboot /
 * fault / IWDG / WWDG / NRST / LPWR, RM0468 §8.4.4) because the CMSIS
 ...
 * their text; log_write_bytes() takes it as a length-delimited run of bytes and
 * never touches the formatter.  Both end in log_append(), the only code that
 * writes a record -- eviction, the SKIP on wrap, NUL and padding, seq before
 * head, and the #13 read-back order exist once, so the two ways in cannot drift
 * apart.
 *
 ...
 * requirement).  Without it an un-read-back'd record is stale on the next boot --
 * the boot-walk reads garbage, judges it malformed and truncates the ring.  These
 * helpers force effectiveness with volatile 32-bit read-backs bracketed by DSB.
 * No HAL / no allocation -- safe from thread, ISR and fault context. */

/* may_alias: these 32-bit read-backs land on the uint8_t data[] written byte-wise
 * by ring_put(); tell the compiler they may alias so strict aliasing does not let
 * it assume the read-backs don't observe those writes. */
...
/* One piece of a record's text: @p n bytes at @p p, not necessarily terminated. */
struct log_seg {
	const char *p;
	uint32_t    n;
};
...
/*
 * THE append.  Every record -- formatted or given as bytes -- is written here and
 * nowhere else.  @p seg holds the text in pieces; their total is cut to
 * LOG_MSG_MAX here too, and the NUL is added here, so no caller has to terminate
 * anything.  Copying the pieces in is the only work done with interrupts masked:
 * nothing in this function formats.
 */
                       ...
                       const struct log_seg *seg, unsigned nseg)
{
	...
	LOG_CRIT_ENTER();
	{
		...
		/* owhinata/wio-lite-ai#13: read the writes back so they durably land in
		 * DTCM (a bare store does not); else the next boot-walk reads stale data
		 * and truncates the ring, dropping this record.  Header last == the commit
		 * point. */
		__DSB();
...
{
	/* Admitted first so that a dropped record costs no formatting; log_append()
	 * asks again, and is the one that decides. */
	if (!log_admit(&level))
		return;
...
/* strnlen() without the libc call: the length of @p s up to @p max, 0 for NULL.
 * Here so that the byte path reaches nothing it does not need. */
                     ...
                     const char *s, size_t len, const char *more)
{
	...
	/* [!] THE LENGTH IS CUT BEFORE ANY ARITHMETIC IS DONE WITH IT.  It comes from
	 * a plugin and can be anything a size_t holds; it is only ever compared until
	 * it is known to fit. */
	if (s == NULL)
		len = 0u;
	if (len <= room) {
