 ...
 * the formatter (issue #112).  @p s need not be terminated; a NUL inside it ends
 * the text there.  When @p prefix and the bytes do not fit in LOG_MSG_MAX, the
 * bytes are cut so that @p more (e.g. " ...") still fits after them, and @p more
 * is appended.  @p prefix / @p more are terminated strings or NULL.  Same record,
 * same ring discipline and same contexts as log_write(): the two share one
 * append.  For text that did not come from this firmware -- a plugin's bytes --
 * where a format string would be the wrong tool.
 */
