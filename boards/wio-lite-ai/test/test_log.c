 ...
 * can be read back directly and compared byte for byte.  The HAL header it
 * includes is log_shim/stm32h7xx_hal.h, which traces the PRIMASK accessors and
 * the barriers; log.c's own LOG_PERSIST_TAP (compiled in by -DLOG_HOST_TEST,
 * out of the firmware) traces the read-backs -- the owhinata/wio-lite-ai#13
 * read-backs whose order is what makes a record survive a reset on this part.
 * So what is asserted is not only what the ring holds afterwards but the
 * ORDER the append does things in.
 *
 ...
 *
 * This file is shared in shape with grove-vision-ai-v2's test_log.c; the
 * log.c they test are separate files (a board port each), so the tests are
 * too.
 */
#include "log.c"
...
uint32_t            log_test_tick;       /* HAL_GetTick() for ts_ms */
struct log_test_rcc log_test_rcc;        /* RCC->RSR for the reset cause */
