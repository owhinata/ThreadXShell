 ...
 * search and argc/argv validation (owhinata/stm32f746g-disco#3).  The parser is a pure function of
 ...
 * and the handler-relative argc/argv and reports a status; the core (owhinata/stm32f746g-disco#4) does
...
/*
 * Minimum capacity of the argv array the core (owhinata/stm32f746g-disco#4) must provide to cli_parse():
...
/**
 * Split @p line on top-level ';' for command sequencing (owhinata/stm32f746g-disco#23).  Returns
...
/*
 * Detect a trailing background '&' on a single command segment (owhinata/stm32f746g-disco#25) and,
...
/*
 * Core line pipeline (owhinata/stm32f746g-disco#4), implemented in cli_session.c.  These functions
...
/** last_result when a running command was cancelled by Ctrl+C (owhinata/stm32f746g-disco#16). */
#define CLI_DISPATCH_CANCELLED (-2)

/*
 * Cooperative-cancel detection (owhinata/stm32f746g-disco#16, cli_session.c, ThreadX-free): drain
 ...
 * cli_prompt() (owhinata/stm32f746g-disco#9).
 */
void cli_edit_session_start(struct cli_instance *sh);

/* Reset the per-session editor state (line / cursor / history / escape parse /
 * render / output-staging / cancel) WITHOUT producing any output (owhinata/stm32f746g-disco#49 P4).
...
/** Set the backspace mode at run time (owhinata/stm32f746g-disco#9): 0 = DEL erases backward,
...
/** Parse and run ONE command segment in place (owhinata/stm32f746g-disco#23/#25): used per-segment
 ...
 * static to cli_edit.c).  owhinata/stm32f746g-disco#10.
 */
void cli_edit_redraw(struct cli_instance *sh);

/**
 * Tab-completion (owhinata/stm32f746g-disco#11, cli_complete.c): complete the word ending at sh->cur
 * against the command set selected by the preceding tokens.  Single match -> insert the remainder
 ...
 * line through add; owhinata/stm32f746g-disco#10 implements the fixed ring here.  The call sites in
 * cli_edit.c / cli_session.c are unchanged -- the only addition owhinata/stm32f746g-disco#10 needs beyond
...
/*
 * Output plumbing (owhinata/stm32f746g-disco#5).  cli_printf.c (ThreadX-free) does the formatting
...
/*
 * Raw binary transfer in progress (owhinata/stm32f746g-disco#50): set by cli_console_claim() for the
...
/*
 * Output bracket (owhinata/stm32f746g-disco#5 lock + owhinata/stm32f746g-disco#25 bg line-break).
 * cli_out_begin takes the output lock (the FOREGROUND's for a bg-job worker, so its output serialises
